...
`--language-order` (default: en,es,fa,hi,ro). Each day:
  ...
  4. Evaluates on ALL 5 language test sets via `pipeline.ft_qwen_day_by_day.eval_sft`
...
Usage:
    python -m pipeline.ft_qwen_day_by_day.train \
        --config pipeline/ft_qwen_day_by_day/configs/qwen3_5_4b_day_by_day.yaml \
...
from pipeline.ft_qwen_day_by_day.prompts import LossMaskingCollator, encode_example
...
def build_training_args(config: dict, extra_cli: list[str]) -> tuple[TrainingArguments, dict]:
    """Same plumbing as pipeline/ft_qwen_day_by_day/train.py: split config-only keys from
...
def run_eval(checkpoint: Path, dataset_dir: str, day_tag: str, tag_prefix: str = "") -> bool:
    ...
    via `python -m pipeline.ft_qwen_day_by_day.eval_sft --checkpoint <day>/final ...`.
    """
    cmd = [
